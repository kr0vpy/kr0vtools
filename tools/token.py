...
def token_generator():
    ...
    print(f"  {D}Crea tokens formato Discord (no funcionales - solo bromas){RS}\n")
    try:
        cant = int(input(f"  {R2}Cuantos{RS} > ").strip())
        if cant < 1: raise ValueError
    except: print(f"  {D}[!] Cantidad invalida{RS}"); return pausa()
    tokens = []
    for i in range(cant):
        tok = generar_token_discord()
        tokens.append(tok)
        print(f"  [{i+1:>3}] {W}{tok}{RS}")
    guardar = input(f"\n  {R2}Guardar en tokens.txt? (s/N){RS} > ").strip().lower()
    if guardar == "s":
        with open("tokens.txt", "w") as f: f.write("\n".join(tokens))
        print(f"  {R1}[+] Guardado en tokens.txt{RS}")
    pausa()
...
def token_verifier():
    ...
    if data and status == 200:
        print(f"  {R1}ESTADO: TOKEN VALIDO{RS}")
        print(f"  {'─' * 40}")
        for label, key in [("ID","id"),("Usuario","username"),("Display","global_name"),
            ("Email","email"),("Telefono","phone"),("Verificado","verified"),
        ...
        try:
            ...
            if billing:
                print(f"\n  {R1}METODOS DE PAGO: {len(billing)}{RS}")
                for b in billing:
                    t = {1: "Credit Card", 2: "PayPal"}.get(b.get('type', 0), "?")
                    print(f"    - {t}")
        except: pass
    elif status == 401: print(f"  {D}ESTADO: TOKEN INVALIDO (HTTP 401){RS}")
    elif status == 403: print(f"  {D}ESTADO: TOKEN BANEADO (HTTP 403){RS}")
    else: print(f"  {D}ESTADO: ERROR ({status}){RS}")
    pausa()

def token_mass_checker():
    limpiar(); barra_menu("DISCORD MASS TOKEN CHECKER")
    path = input(f"  {R2}Ruta del archivo con tokens (1 por linea){RS} > ").strip()
    try:
        with open(path) as f: tokens = [l.strip() for l in f if l.strip()]
    except: print(f"  {D}[!] No se pudo leer el archivo{RS}"); return pausa()
    print(f"  {D}[!] Verificando {len(tokens)} tokens...{RS}\n")
    validos = []
    for i, tok in enumerate(tokens):
        data, status = verificar_token(tok)
        if status == 200:
            estado = f"{R1}VALIDO{RS}"
            user = f" | {data['username']}#{data.get('discriminator', '0')}" if data else ""
            validos.append((tok, data))
        elif status == 401: estado = f"{D}INVALIDO{RS}"; user = ""
        else: estado = f"{D}ERROR ({status}){RS}"; user = ""
        print(f"  [{i+1:>4}] {estado} {W}{tok[:50]}...{RS}{user}")
        barra_progreso(i+1, len(tokens), prefijo="Checking")
    print(f"\n  {R1}[+] {len(validos)}/{len(tokens)} tokens validos{RS}")
    if validos:
        guardar = input(f"  {R2}Guardar tokens validos? (s/N){RS} > ").strip().lower()
        if guardar == "s":
            with open("valid_tokens.txt", "w") as f:
                for t, d in validos: f.write(f"{t} | {d['username']}#{d.get('discriminator', '0')}\n")
            print(f"  {R1}[+] Guardado en valid_tokens.txt{RS}")
    pausa()
...
def token_nitro_checker():
    ...
    print()
    data, status = verificar_token(token)
    if not data: print(f"  {D}[!] Token invalido{RS}"); return pausa()
    headers = {"Authorization": token, "User-Agent": UA}
    try:
        ...
        if subs:
            for s in subs:
                ...
                if s.get('current_period_end'):
                    ...
                    print(f"  {R2}Expira:{RS} {fin}")
        else: print(f"  {D}[!] No tiene suscripciones activas{RS}")
    except urllib.error.HTTPError as e: print(f"  {D}[!] HTTP {e.code}{RS}" if e.code != 403 else f"  {D}[!] Sin permisos{RS}")
...
def token_decoder():
    ...
    if not token: return pausa()
    partes = token.split(".")
    if len(partes) != 3: print(f"  {D}[!] Formato de token invalido{RS}"); return pausa()
    try:
    ...
    except Exception as e: print(f"  {D}[!] Error decodificando: {e}{RS}")
    pausa()
...
def menu_token():
    while True:
        try:
            ...
            print()
            menu_en_columnas([("1","Generator"),("2","Verifier"),("3","Mass Checker"),("4","Nitro Checker"),("5","Decoder"),("0","Volver")])
            print()
